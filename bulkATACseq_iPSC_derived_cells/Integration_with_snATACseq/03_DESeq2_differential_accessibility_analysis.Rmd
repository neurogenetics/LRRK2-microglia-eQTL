---
title: "DESeq2_differential_accessibility_analysis_snATAC_bulkATAC"
...
```{r}
counts <- read.csv("./output/snATACseq_bulkATAC_celltype_counts_summ.csv", header = T, row.names = NULL)
...
celltype <- factor(c("Astrocyte", "Endothelial Cell", rep("Glutamatergic Neuron", 3), rep("GABAergic Neuron", 4), "Microglia", rep("Oligodendrocyte", 3), "Oligodendrocyte Precursor Cell", rep("iMicroglia", 4), rep("iFbN", 3)))

source <- factor(c(rep("Brain_FC", 14), rep("iPSC_PPMI", 4), rep("iPSC_33i", 3)))
...
## Genome-wide accessibility correlation iMGL.3411 vs brain MGL.4
```{r}
...
pheatmap(rld.cor, annotation = coldata,
